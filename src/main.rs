...
async fn main() {
    ...
    let base_url : &str = "synt2x.xyz";
    let mut setup_url : &str = "setup.synt2x.xyz";
   ...
   {} | Build Date: {} | Version: {}", base_url ,build_date, env!("CARGO_PKG_VERSION"));
    ...
    if terminal_width < 80 {
        print!("{}\n", format!("SYNTAX 2 Bootstrapper | {} | Build Date: {} | Version: {}", base_url, build_date, env!("CARGO_PKG_VERSION")).to_string().magenta().cyan().italic().on_black()); // Fallback message
    } else {
...
Terminal=true
Version={}
MimeType=x-scheme-handler/synt2x-player;", latest_bootstrapper_path.to_str().unwrap(), latest_bootstrapper_path.to_str().unwrap(), env!("CARGO_PKG_VERSION"));
                
                let desktop_file_path = dirs::data_local_dir().unwrap().join("applications").join("synt2x-player.desktop");
    ...
    if !app_settings_path.exists() { //|| !client_executable_path.exists() {
        ...
        // Install the synt2x-player scheme in the registry
        info("Installing synt2x-player scheme");
        #[cfg(target_os = "windows")]
        {
            ...
            let hkey_syntax_player = hkey_classes_root.create_subkey("synt2x-player").unwrap().0;
...
<Settings>
    ...
    // Looks something like "synt2x-player://1+launchmode:play+gameinfo:TICKET+placelauncherurl:https://synt2x.xyz/Game/placelauncher.ashx?placeId=660&t=TICKET+k:l"
    debug(&format!("Arguments Passed: {}", args.join(" ").bright_blue()));
    if args.len() == 1 {
        // Just open the website
        #[cfg(target_os = "windows")]
        {
            std::process::Command::new("cmd").arg("/c").arg("start").arg("https://synt2x.xyz/games").spawn().unwrap();
            std::process::exit(0);
        }
        #[cfg(not(target_os = "windows"))]
        {
            std::process::Command::new("xdg-open").arg("https://synt2x.xyz/games").spawn().unwrap();
            std::process::exit(0);
        }
    }

    let main_args = &args[1];
    let main_args = main_args.replace("synt2x-player://", "");
